...
class PowerManagerService extends IPowerManager.Stub
    ...
    /*
     * by guobaozu for saving overlay status before suspend 
     * the restore operation is in the keyguardViewManager.java
     */
    ...
    */
    ...
    private int setScreenStateLocked(boolean on) {
        ...
        if (err == 0) {
            ...
            /***** added by guobaozu for saving the overlay status *****/
            if(!on) {
            	/* the best way is DO NOT judge the overlay status on lock screen ui */
            	if(!mPolicy.inKeyguardRestrictedKeyInputMode()) { 
	        		mOverlay.check_status(0);
	        		mOverlay.close(0);
	        		mOverlay.lock(0,1);
            	}
            }
