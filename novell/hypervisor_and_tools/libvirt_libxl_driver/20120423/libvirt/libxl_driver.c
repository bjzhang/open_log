...
//            chr->targetType == VIR_DOMAIN_CHR_CONSOLE_TARGET_TYPE_VIRTIO) {
            libxl_get_console_tty(&priv->ctx, domid, &pts);
            chr->source.data.file.path = (char *)malloc(20);
            sprintf(chr->source.data.file.path, "/dev/pts/%d", pts);
//        }
        VIR_DEBUG("tty: %s", chr->source.data.file.path);
    }
...
static virDriver libxlDriver = {
    ...
    .domainOpenConsole = libxlDomainOpenConsole, /* 0.9.6 */
};
