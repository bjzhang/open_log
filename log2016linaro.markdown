...
3.  这次LCE有个环节是"ask maintainer/experter", 会安排15分钟和maintainer交流的机会(如果没有下一个人,其实可以交流更长时间). 和suse的alex graf, jiri kosina交流, 想和gregkh交流没找到时间.
...
4.  和欧研交流
    欧研较多工程师来参加linuxcon, 一起讨论了memory和存储的一些问题.
    Pradeep提到IO NUMA, 把IO请求调度给临近的cpu处理, 感觉对于我司的芯片场景很有帮助.

5.  有趣的session
    1.  OpenQA
        openQA在sle各组件都在用，除了用opencv也会比较串口信息。现在suse/opensue全部用openQA测试，fedora也用。openQA支持多种架构。
        和richard brown简单聊了下openQA, 由于时间关系没法去听session，建议eulerOS关注。
        原来以为openQA只是用openCV抓图比较图片，其实现在也可以抓串口数据。基本来说suse所有产品，opensuse，fedora都使用openQA用于测试，据说suse所有的自动化用例都加进去了。只有个别UI用例还是手动的。强烈建议做发行版的童鞋关注。

    2.  unikernel
        unikernel by amir(docker)
        ...
        When should you use unikernel?

        认为可用，就像早期汽车一样，用户需要有比较多的知识。

        问下arm64的支持状态。

        问题：device drvier：其中一个使用xen的原因，就是unikernel在domU，在dom0提供驱动。
        问题：好像提到了rumpkernel？

    3.  区块链
    4.  keynote4(ARM): 感觉arm现在影响力也挺大的。在第一天的keynote里面。
        主要讲的区块链. 这次linuxcon另有两个session也讲的是区块链.
        www.openchainproject.org new collaboration project in linux foundation.
...
6.  其他session:
    ...
    3.  keynote3 SDS huawei
        字体太多, 后面观众可能看不清楚.
        linuxcon有个openSDS panel discussion, 包括dell, EMC, huawei, intel等公司. 时间关系没去听.
    5.  keynote5: intel vm and container
        主要讲的是intel clear container, 我司的Floria童鞋在演讲中比较了clear container和安全容器技术的区别.

    5.  session: kernel regression
        work on the regression report for 4.7 and 4.8.
        主要是面向一般用户讲什么是regresstion，和bug有什么区别。如何判断是否是regression。如何报告regression。

    6.  Linaro Alex shi继续介绍内核补丁回合方法和技巧, 里面涉及到的git用法觉得可以给大家科普下.
