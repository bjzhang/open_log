...
Syscall is the interface between kernel and userspace. The correctness and stability of syscall is very important to the system, especially for new architecture or new abi of kernel and glibc. The existence fuzz test such as trinity, skyaller could only generate the random or boundary value of function parameter and inject into system but they could not generate each item in the complext types and will not check whether these parameter are passed to the real function of syscall.
Bamvor Jian Zhang would like to introduce a new fuzz tools which overcome the shortage of current fuzz tools. By generate the each item in the complex data type and check the corrsponding value in kernel side, the tester could ensure the all the parameter pass correctly which is important for porting and do the real fuzz to the system which may potentially increasing the coverage of kernel.
...
The audience is anyone interested in Kernel/libc porting and/or gesting. Attendees can understand the fuzz tools and how to use these fuzz tools in their environment. As whole package, such fuzz tools provide automaically generated function helper for generating the specific parameter to function, and the jprobe function hook for monitor all the syscall given by the syscall list in kernel. The QA engineer could integrated those tools into continuous integration. And the developer could enhance or extend the functionality without stuck at hundreds of syscall and dezens of struct.
...
This presentation will help developper to evaluate the changes of syscall in kernel or changes of syscall wrapper in glibc. whether the changes is safe. will help the porting to kernel and/or glibc to new architecture or abi.

The idea behind these fuzz tools grows up when bamvor encounter lots of glibc issue of current aarch64 ILP32 implementation. It is indeed a painful work when checking the parameter passing from libc library to real kernel syscall function. It is hard to evaluate such porting due to lack of the unit test of syscall. The existing tools such as trinity, skykaller, ltp could not help on this senario.
