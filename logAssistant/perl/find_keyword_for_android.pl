...
$opt_f = $path."/".$file;
#$opt_f = $path."/log201101vimicro.txt";
...
#						print "match $keyword_utf8 in $keyword_line\n";						
            ...
            my $content_key_utf8 = $content_key;
            if ( $cur_env_locale eq "gb2312" ) {
                $content_key_utf8 = Encode::encode("utf8", $content_key);
            }
            my $content_match = 0;
            if ( $opt_i ) {
...
$droid->view("file://".$output_file,"text/plain");
